...
test("reunion reuses the selected destination local event as a deterministic NPC clue", () => {
  ...
  assert.equal(first.npcName, "マルコ");
  assert.equal(first.eventId, localEvent.id);
  assert.equal(first.eventTitle, localEvent.title);
  assert.match(first.text, /マルコはこの辺りの話を教えてくれた/);
  assert.ok(first.text.includes(localEvent.hook));
});

test("another destination and travel-window boundaries do not show a reunion or clue", () => {
  const root = rootWithKnowledge();
  assert.equal(Presentation.reunionForEntry(root, knownDestinations["sim:old-forest"], new Date(2026, 8, 1, 11)), null);
  assert.equal(Presentation.reunionClueForEntry(root, knownDestinations["sim:old-forest"], new Date(2026, 8, 1, 11)), null);
  assert.equal(Presentation.reunionForEntry(root, knownDestinations["sim:north-road-ford"], new Date(2026, 8, 1, 8)), null);
  assert.equal(Presentation.reunionClueForEntry(root, knownDestinations["sim:north-road-ford"], new Date(2026, 8, 1, 8)), null);
  assert.equal(Presentation.reunionForEntry(root, knownDestinations["sim:north-road-ford"], new Date(2026, 8, 1, 15)), null);
  assert.equal(Presentation.reunionClueForEntry(root, knownDestinations["sim:north-road-ford"], new Date(2026, 8, 1, 15)), null);
});

test("Atlas reunion presentation reuses encounter and local-event domains without persistence or coordinates", () => {
  ...
  assert.match(source, /CrownlessDiscoveryActions/);
