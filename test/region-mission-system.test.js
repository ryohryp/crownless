...
test('geographic road tags use only the coarse exploration area for mission identity', () => {
  ...
  state.worldKnowledge = { discoveries: {
    'geo:way/123:encounter:road_hub': {
      key: 'geo:way/123:encounter:road_hub', name: '黒土の辻', baseTitle: '黒土の辻', terrain: ['road_hub'], contentKind: 'encounter', state: 'discovered', firstDiscoveredAt: 1000, visits: 1, areaId: 'area:14:14555:6451'
    }
  } };
  const discovery = {
    id: 'geo-road-test', locationId: 'watchfire-hill', discoveryKey: 'geo:way/123:encounter:road_hub',
    geographicDiscovery: { features: ['road_hub'], palette: 'road', sourceRef: 'way/123', representativeCoordinate: { latitude: 35.0, longitude: 139.0 } }
  };
...
test('unrelated simulated discoveries do not start the road mission', () => {
  ...
  const result = Core.recordRegionMissionClue(state, { id: 'ruined-chapel', locationId: 'ruined-chapel', palette: 'chapel' }, 1000);
  assert.equal(result.changed, false);
  assert.deepEqual(Core.getRegionMissionBoard(state), []);
});

test('regional mission system remains valid JavaScript', () => {
  execFileSync(process.execPath, ['--check', path.join(__dirname, '..', 'src', 'region-mission-system.js')]);
});
