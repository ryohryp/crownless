...
test("traveling NPC becomes a coarse unconfirmed nearby signal only while traveling", () => {
  const morning = Signals.travelingSignals(NpcLife, 8);
...
test("presentation marks NPC signal as rumor rather than a confirmed discovery", () => {
  assert.match(source, /data\.atlasSignalSource = "npc-rumor"/);
