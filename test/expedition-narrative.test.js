...
function syntheticCombat(result = "victory") {
  return {
    encounterId: "grey-wolves", encounterName: "灰狼", enemyCount: 4, initialEnemyCount: 4,
    remainingEnemyCount: result === "victory" ? 0 : 1, enemyTags: ["beast"], result,
    hpBefore: 100, hpAfter: result === "defeat" ? 0 : result === "retreat" ? 28 : 61, maxHp: 100,
    damage: result === "defeat" ? 100 : result === "retreat" ? 72 : 39, healed: 0,
...
test("same resolved report produces the same narrative without mutating combat state", () => {
  const report = { expeditionId: "exp-1", seed: 44, companionIds: ["mira", "ed"], policyId: "standard", policyName: "通常", combat: { encounters: [syntheticCombat()] } };
  const before = JSON.parse(JSON.stringify(report));
  const input = { report, companions: [mira, ed, sella], policies: system.policies };
  assert.deepEqual(narrative.buildExpeditionNarrative(input), narrative.buildExpeditionNarrative(input));
...
test("narrative events retain structured actor enemy consequence HP and cause data", () => {
  ...
  battle.lines.forEach((line) => {
    assert.ok(line.phase); assert.ok(line.actorId); assert.equal(line.enemyId, "grey-wolves"); assert.ok(line.action); assert.ok(line.reaction); assert.ok(line.consequence);
    assert.equal(typeof line.hpBefore, "number"); assert.equal(typeof line.hpAfter, "number"); assert.ok(Array.isArray(line.causes)); assert.ok(line.text.length > 0);
  });
});
...
test("battle arc compresses raw rounds into opening pressure turning point and finish", () => {
  ...
  combat.remainingEnemyCount = 0; combat.hpAfter = 57;
  ...
  assert.equal(phases[0], "opening"); assert.ok(phases.includes("pressure")); assert.ok(phases.includes("turning-point")); assert.ok(phases.includes("finish"));
...
test("Mira Ed and Sella produce visibly different battle voices", () => {
  ...
  const sellaCombat = syntheticCombat(); sellaCombat.causes = ["conceal", "greedy"]; sellaCombat.rounds[0].events = ["ambush"];
  ...
  assert.match(text(miraBattle), /ミラ/); assert.match(text(edBattle), /エド/); assert.match(text(sellaBattle), /セラ/);
  assert.notEqual(text(miraBattle), text(edBattle)); assert.notEqual(text(edBattle), text(sellaBattle));
});
...
test("trait and equipment causes read as authored events instead of system jargon", () => {
  ...
  assert.match(text, /弓|藪|包囲|狼|灰狼/); assert.match(text, /エド|押し|刃|前へ/);
...
test("victory retreat and defeat finish with different aftertastes", () => {
  ...
  assert.notEqual(finish(victory), finish(retreat)); assert.notEqual(finish(retreat), finish(defeat));
  assert.match(finish(retreat), /ここまで|帰る|距離を切/); assert.match(finish(defeat), /支えきれ|膝|崩れ/);
});
...
test("report UI uses narrative through the scroll only and keeps raw chronology collapsed", () => {
  ...
  assert.match(presentation, /EXPEDITION SCENES/); assert.match(presentation, /遠征絵巻/);
  assert.doesNotMatch(presentation, /BATTLE NARRATIVE/); assert.doesNotMatch(presentation, /遠征記/); assert.doesNotMatch(presentation, /renderBattleNarrative/);
  assert.doesNotMatch(css, /\.expedition-narrative/); assert.match(presentation, /遠征の記録を時系列で読む/); assert.doesNotMatch(presentation, /時系列と戦闘数値を確認する/);
  ...
  assert.match(reportBody, /details\.open = false/); assert.doesNotMatch(reportBody, /details\.open = true/);
});
...
test("active expedition UI progressively reveals only elapsed log entries", () => {
  ...
  assert.match(presentation, /function activeLogEntries/); assert.match(presentation, /const progress = elapsed \/ duration/); assert.match(presentation, /const preview = system\.resolveExpedition\(expedition, state\)/);
  assert.match(presentation, /entry\.type !== "return" && entry\.minute <= cutoffMinute/); assert.match(presentation, /liveTime: formatLiveClock/); assert.match(presentation, /遠征中の記録/); assert.match(presentation, /最新の記録を確認する/); assert.match(presentation, /details\.open = true/);
});
