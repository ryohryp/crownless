...
test("issue #582 Loot Hunt phone-size runtime gate", async ({ page }) => {
  ...
  const pathCopyBox = await page.locator(".path-copy").boundingBox();
  const lastValue = comparison.locator(".loot-comparison-rows strong").last();
  const lastValueBox = await lastValue.boundingBox();
  ...
  expect(pathCopyBox).not.toBeNull();
  expect(lastValueBox).not.toBeNull();
  expect(comparisonBox.y).toBeGreaterThanOrEqual(pathCopyBox.y - 1);
  expect(lastValueBox.y + lastValueBox.height).toBeLessThanOrEqual(pathCopyBox.y + pathCopyBox.height + 1);
  ...
  observation.cleared = {
    ...
    pathCopyBox,
    lastValueBox,
    fullComparisonFitsLocalViewport: true,
