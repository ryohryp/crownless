(() => {
  ...
  function syncExplorationSource() { const presentation = window.CrownlessExplorationMap; if (!presentation || typeof presentation.setDiscoverySource !== "function") return; presentation.setDiscoverySource(document, locationState === "ready" && geographicDiscoveries.length ? "geographic" : "simulated"); }
  ...
  Core.discoverLocation = function discoverLocationWithGeography(state, choiceId) { return recordWorldKnowledge(applySelectedGeographicDiscovery(originalDiscoverLocation(state, choiceId), choiceId)); };
