(() => {
  ...
  function emptyDiagnostics(gps = "idle") {
    return {
      ...
      gpsMessage: "",
      gpsElapsedMs: null,
      gpsOptions: Object.assign({}, GEOLOCATION_OPTIONS),
      osm: "idle",
      endpoint: "-",
      attempt: 0,
      total: 0,
  ...
  function diagnosticText() {
    const osmDetail = diagnostics.endpoint && diagnostics.endpoint !== "-"
      ? `${diagnostics.osm}@${endpointLabel(diagnostics.endpoint)}`
      : diagnostics.osm || "-";
    const gpsDetail = diagnostics.gpsCode
      ? `${diagnostics.gps}(${diagnostics.gpsCode}:${diagnostics.gpsName})`
      : diagnostics.gps;
  ...
  async function performGeographicDiscovery() {
    let provider = null;
    try {
  ...
  function beginGeographicDiscoveryAfterPaint() {
    return new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))).then(performGeographicDiscovery);
  }

  function loadGeographicDiscoveries() {
    if (locationPromise) return locationPromise;
    locationState = "loading";
    diagnostics = emptyDiagnostics("requesting");
    setPendingUi();
    locationPromise = beginGeographicDiscoveryAfterPaint();
    return locationPromise;
  }

  function reloadGeographicDiscoveries() {
  ...
  // New expeditions are the lifecycle boundary. Do not couple location discovery
  // to a specific button: both the first departure and "return-again" flow call
  // Core.beginExpedition through the app's shared beginNewExpedition path.
  if (originalBeginExpedition) {
    Core.beginExpedition = function beginExpeditionWithLocationDiscovery(...args) {
      const next = originalBeginExpedition(...args);
      reloadGeographicDiscoveries();
      return next;
    };
  }
  ...
  window.CrownlessLocationDiscoveryRuntime = {
    ...
    get diagnostics() {
      return Object.assign({}, diagnostics, {
        gpsOptions: Object.assign({}, diagnostics.gpsOptions),
        features: diagnostics.features.slice(),
        names: diagnostics.names.slice()
      });
    },
