...
(() => {
  ...
  function gearPanel() {
    return `<p class="kicker">MAKE IT HOME. MAKE IT YOURS.</p><h2>次の旅の、戦い方。</h2><p class="small">装備は生還して初めて手に入る。失敗しても、持っていた装備は失わない。</p><div class="gear-list">${state.owned.filter(g => g !== 'crown').map(g => button('equip',`${E.GEAR[g].name}${state.equipped === g ? ' · 装備中' : ''}`,E.GEAR[g].text,{value:g,class:`choice ${state.equipped === g ? 'selected' : ''}`})).join('')}</div>${state.owned.includes('crown') ? '<p class="badge">灰の王冠 · 永続で最大体力 +6</p>' : ''}<div class="rule-line"><div class="section-heading"><h3 style="margin:0">旅装を補強する</h3><span class="small">${state.level} / 4</span></div><p class="small">最大体力 +5。毎回の遠征で効果が続く。</p>${button('upgrade',state.level >= 4 ? '補強を終えた' : `鉄片 ${E.upgradeCost(state)} で補強する`,'',{class:'secondary',disabled:state.level >= 4 || state.scrap < E.upgradeCost(state)})}${notice ? `<p class="notice" role="status">${esc(notice)}</p>` : ''}</div>`;
  }
  function camp() {
  ...
  function fight(x) {
    const e = x.enemy, next = E.intent(e), attack = E.GEAR[state.equipped].attack + x.focus;
    const hit = amount => Math.max(0, amount - (next.id === 'guard' ? 5 : 0));
    const strong = state.equipped === 'bow' ? attack + 4 : hit(attack+4);
    return `<p class="kicker">${e.elite ? 'GUARDIAN' : 'ENCOUNTER'} / ${E.GEAR[state.equipped].name}</p><div class="hp-row"><h2 style="margin:0">${e.elite ? '主・' : ''}${E.ENEMIES[e.kind].name}</h2><span>${e.hp} <small class="small">/ ${e.maxHp}</small></span></div><div class="bar enemy-bar" role="meter" aria-label="敵の体力" aria-valuenow="${e.hp}" aria-valuemin="0" aria-valuemax="${e.maxHp}"><span style="width:${e.hp/e.maxHp*100}%"></span></div><div class="intent"><p class="kicker">次の行動 · 行動を選ぶまで時間は進まない</p><span class="damage">${next.damage ? next.damage : '—'}</span><strong>${next.name}</strong><small>${next.help}</small></div><div class="choice-grid">${button('strike',`${state.equipped === 'bow' ? '射る' : '斬る'} <span class="cost">${hit(attack)}</span>`,'気力 +1 / 表示は与えるダメージ')}${button('heavy',`強撃 <span class="cost">${strong}</span>`,'気力 −2 / 大きな一撃',{disabled:x.stamina < 2})}${button('guard','防御',`気力 +1 / ${state.equipped === 'shield' ? '12 軽減・3 反撃' : '9 ダメージ軽減'}`)}${button('dodge','回避',`気力 −1 / 無傷・次の攻撃 +${state.equipped === 'fang' ? 5 : 3}`,{disabled:x.stamina < 1})}</div>${logs(x)}<div class="combat-foot">${button('heal',`薬草 ${x.potions} · 体力 +12`,'敵も行動する',{class:'choice',disabled:x.potions === 0 || x.hp === E.maxHp(state)})}${button('flee',`撤退 · 体力 −${Math.max(2,next.damage)}`,x.hp <= Math.max(2,next.damage) ? '生還できない' : '残れば戦利品を持ち帰れる',{class:'choice',disabled:x.hp <= Math.max(2,next.damage)})}</div><p class="small" style="margin:12px 0 0">背嚢：鉄片 ${x.scrap}${x.gear.length ? ' / 装備 '+x.gear.length+' 個' : ''} · 生還で確定${x.focus ? ` / 追撃 +${x.focus}` : ''}</p>`;
  }
  ...
  root.addEventListener('click', event => {
    ...
    else if (action === 'upgrade') { state = E.upgrade(state); if (state !== before) notice = `旅装を補強した。最大体力 ${E.maxHp(state)}。`; }
