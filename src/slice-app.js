...
(() => {
  ...
  const ledger = x => `<div class="loot-ledger"><p class="kicker">AT RISK · 生還で確定</p><strong>${x.scrap}</strong> <small>鉄片 / 背嚢の中</small>${x.gear.length ? `<p><small data-current-gear-id="${state.equipped}">現在装備：${E.GEAR[state.equipped].name} · ${E.gearText(state,state.equipped)}</small></p>` : ''}${x.gear.map(g => `<p>＋ ${E.GEAR[g].name}<br><small>未帰還 · ${E.gearText(state,g)}</small></p>`).join('')}</div>`;
  function onboard() {
