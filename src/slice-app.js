...
(() => {
  ...
  function mapPins() {
    return `<div class="exploration-map" aria-label="霧を晴らして開拓する探索地図"><div class="places">${E.PLACES.map(p => {
      const unlocked = state.unlocked.includes(p.id), cleared = state.cleared.includes(p.id);
      const status = !unlocked ? '未探索' : cleared ? '踏破済み' : '未知の気配';
      return `<button class="place-pin ${selected === p.id ? 'selected' : ''} ${unlocked ? 'discovered' : 'locked'}" data-action="select" data-value="${p.id}" aria-pressed="${selected === p.id}">${A.icon(p.id)}<span>${unlocked ? p.name : '霧の向こう'}</span><small class="map-state">${status}</small></button>`;
    }).join('')}</div></div><p class="atlas-note">${state.unlocked.length} / 4 の土地を発見 · 歩くほど霧が晴れ、見つけた土地から遠征できます。</p>`;
  }
  function scouting() {
