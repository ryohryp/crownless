...
(() => {
  ...
  const ledger = x => `<div class="loot-ledger"><p class="kicker">AT RISK · 生還で確定</p><strong>${x.scrap}</strong> <small>鉄片 / 背嚢の中</small>${x.gear.length ? `<p><small data-current-gear-id="${state.equipped}">現在装備：${E.GEAR[state.equipped].name} · ${E.gearText(state,state.equipped)}</small></p>` : ''}${x.gear.map(g => `<p data-found-gear-id="${g}">＋ ${E.GEAR[g].name}<br><small>未帰還 · ${E.gearText(state,g)}</small></p>`).join('')}</div>`;
  function onboard() {
  ...
  function gearPanel() {
    ...
    const retry = retryPlace
      ? `<div class="button-stack">${button('redepart',`この装備で${retryPlace.name}へもう一度`,`${E.GEAR[id].name}を試す / すぐ再遠征`,{class:'primary',value:retryPlace.id})}</div>`
      : '';
  ...
  function pathPanel(x) {
    ...
    const decisions = clear ? `${button('return','戦利品を持って帰る','',{class:'primary'})}${x.depth < 3 ? `<p class="notice">${E.lootCue(x.place,x.depth+1)}</p>${button('deeper',`深層 ${x.depth+1} へ踏み込む`,`敵の行動も変化 / 珍しい武具の可能性 / 鉄片 ×${x.depth+1}`)}` : '<p class="small">最深部へ到達した。火のもとへ帰ろう。</p>'}` : event ? `${button('rest','火のそばで休む','体力 +6 / 遺品は残す')}${button('search',`茨の遺品を拾う`,`体力 −4 / 鉄片 +${5*x.depth}`,{disabled:x.hp <= 4})}` : `${button('careful',x.room === 4 ? '主に挑む' : '静かに足跡をたどる','通常の敵 / 体力を温存したい')}${button('risky','宝の気配を追う',x.depth >= 2 ? '敵の体力 +3 / 鉄片 +3 / 珍しい武具の可能性' : '敵の体力 +3 / 鉄片 +3')}`;
  ...
  function report() {
    ...
    const recoveryParts = recovery
      ? [recovery.gear ? E.GEAR[recovery.gear]?.name : '', recovery.scrap ? `鉄片 ${recovery.scrap}` : ''].filter(Boolean)
      : [];
  ...
  root.addEventListener('click', event => {
    ...
    else if (action === 'redepart') { selected = value; locationRequest++; busy = false; state = E.start(state,value); if (state !== before) lastReturnedPlace = null; }
    ...
    if (['depart','redepart','continue','return','flee'].includes(action) || (before.expedition && state.expedition?.stage !== before.expedition.stage)) {
