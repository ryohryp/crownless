...
(() => {
  ...
  const ledger = x => `<div class="loot-ledger"><p class="kicker">AT RISK · 生還で確定</p><strong>${x.scrap}</strong> <small>鉄片 / 背嚢の中</small>${x.gear.length ? `<p><small>現在装備：${E.GEAR[state.equipped].name} · ${E.gearText(state,state.equipped)}</small></p>` : ''}${x.gear.map(g => `<p>＋ <strong>${E.GEAR[g].name}</strong><br><small>未帰還 · ${E.gearText(state,g)}</small><br><small>現在：${E.GEAR[state.equipped].name} · ${E.gearText(state,state.equipped)}</small></p>`).join('')}</div>`;
  function onboard() {
