...
(() => {
  ...
  function gearOrigin(id) {
    if (id === 'rust') return null;
    const family = E.gearFamily(id);
    return E.PLACES.find(p => E.gearFamily(p.weapon) === family) || null;
  }
  function placeMemory(p) {
    if (!state.cleared.includes(p.id)) return '';
    const gear = state.owned.find(id => gearOrigin(id)?.id === p.id);
    const unresolved = p.id === 'crypt' ? '王墓には、まだ語られていないものがある。' : E.lootCue(p.id, 2);
    return `<div class="rule-line"><p class="kicker">旅の記憶</p><p class="small">${E.ENEMIES[p.enemy].name}を越えた。${gear ? ` ${E.GEAR[gear].name}をここから持ち帰った。` : ''}</p><p class="notice">${unresolved}</p></div>`;
  }
  function explorePanel() {
    const p = E.place(selected), unlocked = state.unlocked.includes(p.id), locked = p.id === 'crypt' && state.cleared.length < 2;
    return `<p class="kicker">${unlocked ? p.terrain : 'UNDISCOVERED'}</p><h2>${unlocked ? p.name : 'まだ、霧の向こう。'}</h2><p class="intro">${unlocked ? p.subtitle : 'いつもと違う道を歩くと、別の土地に出会えるかもしれない。発見した場所には、あとから何度でも遠征できる。'}</p>${unlocked ? `<div class="reward"><span class="reward-icon">♢</span><div><strong>${p.reward}</strong><small>${p.hint}</small></div></div><p class="small">5 つの場面 / 戦闘 3 回 / 休息 2 回<br>${p.id === 'crypt' ? '危険度：高い。装備と体力を整えてから。' : '初回は 3〜5 分。深層では珍しい武具が出ることがある。'}</p>${state.cleared.includes(p.id) ? '<span class="badge">踏破済み · 深層で珍しい武具を探せる</span>' : ''}${placeMemory(p)}<div class="button-stack">${button('depart',locked ? `他の土地をあと ${2-state.cleared.length} か所踏破する` : 'この土地へ遠征する','',{class:'primary',value:p.id,disabled:locked})}</div>` : ''}${scouting()}`;
  }
  function gearPanel() {
    const id = state.equipped, level = E.weaponLevel(state,id), cost = E.upgradeCost(state,id);
    return `<p class="kicker">MAKE IT HOME. MAKE IT YOURS.</p><h2>次の旅の、戦い方。</h2><p class="small">深層では同じ武器種でも戦い方の違う一本が見つかる。補強は一本ごとに残る。</p><div class="gear-list">${state.owned.filter(g => g !== 'crown').map(g => { const origin = gearOrigin(g); return button('equip',`${E.GEAR[g].name}${state.equipped === g ? ' · 装備中' : ''} · 補強 ${E.weaponLevel(state,g)}/4`,`${E.gearText(state,g)}${origin ? ` ${origin.name}の戦利品。` : ''}`,{value:g,class:`choice ${state.equipped === g ? 'selected' : ''}`}); }).join('')}</div>${state.owned.includes('crown') ? '<p class="badge">灰の王冠 · 灰冠の廟の戦利品 · 永続で最大体力 +6</p>' : ''}<div class="rule-line"><div class="section-heading"><h3 style="margin:0">${E.GEAR[id].name}を補強する</h3><span class="small">${level} / 4</span></div><p class="small">この一本の得意行動だけが一段強くなる。別の武器には影響しない。</p>${button('upgrade',level >= 4 ? 'この武器の補強を終えた' : `鉄片 ${cost} で補強する`,'',{class:'secondary',disabled:level >= 4 || state.scrap < cost})}${notice ? `<p class="notice" role="status">${esc(notice)}</p>` : ''}</div>`;
  }
  function camp() {
  ...
  function pathPanel(x) {
    ...
    return `<p class="kicker">${clear ? 'A WAY HOME' : 'ONE MORE ROOM?'}</p><h2>${title}</h2><p class="intro">${clear ? '手に入れたものを、焚き火へ。まだ余力があるなら、より危険な深層へ進むこともできる。' : event ? '息を整えるか、傷を引き受けて遺品を拾うか。引き返す道も、まだ残っている。' : x.room === 4 ? `この土地の主が奥を守っている。深層では、まだ見ていない武具を持つ主もいる。` : '静かな道をたどるか、宝の気配を追うか。深く踏み込むほど、敵の読み方も変わる。'}</p>${ledger(x)}${cue}<div class="button-stack">${clear ? `${button('return','戦利品を持って生還する','',{class:'primary'})}${x.depth < 3 ? `<p class="notice">${E.lootCue(x.place,x.depth+1)}</p>${button('deeper',`深層 ${x.depth+1} へ踏み込む`,`敵の行動も変化 / 珍しい武具の可能性 / 鉄片 ×${x.depth+1}`)}` : '<p class="small">最深部へ到達した。火のもとへ帰ろう。</p>'}` : event ? `${button('rest','火のそばで休む','体力 +6 / 遺品は残す')}${button('search',`茨の遺品を拾う`,`体力 −4 / 鉄片 +${5*x.depth}`,{disabled:x.hp <= 4})}` : `${button('careful',x.room === 4 ? '主に挑む' : '静かに足跡をたどる','通常の敵 / 体力を温存したい')}${button('risky','宝の気配を追う',x.depth >= 2 ? '敵の体力 +3 / 鉄片 +3 / 珍しい武具の可能性' : '敵の体力 +3 / 鉄片 +3')}`}</div>${logs(x)}${!clear ? `${button('return','ここで生還する','',{class:'secondary'})}<div class="combat-foot">${button('heal',`薬草を使う（残り ${x.potions}）· 体力 +12`,'',{class:'text-button',disabled:x.potions === 0 || x.hp === E.maxHp(state)})}<span class="small">安全に使える</span></div>` : ''}`;
  }
  function expedition() {
