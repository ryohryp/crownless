(function (root, factory) {
  ...
  function createResilientStorage(nativeStorage, onStatus) {
    ...
    return Object.freeze({
      ...
      removeItem(key) {
        const id = String(key);
        memory.delete(id);
        nativeCall('removeItem', [id], undefined);
      },
      clear() {
        memory.clear();
  ...
  function placeMemory(state, id, title, text) {
    return Object.freeze({ id, title, text, state: state.placeStates[id] || 'unknown' });
  }
  ...
  function buildCompletion(inputState) {
    ...
    const memories = [
      ...
      placeMemory(state, p3.BLACK_RAVEN_HILL, '黒鴉の丘', hill === p3.CHOICES.SIGNAL_CHAPEL
      ...
      placeMemory(state, p5.COLLISION_PLACE, collision.title, `${collision.summary} ${collision.history}`)
    ];
    return Object.freeze({
  ...
  function installStyles(doc) {
    ...
    style.textContent = `
      ...
      .reboot-ending .ending-branch{font-size:.78rem;opacity:.72;overflow-wrap:anywhere}.land-memory-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.65rem;margin-top:1rem}
      .land-memory-card{padding:.75rem;border:1px solid rgba(60,48,32,.22);background:rgba(255,252,241,.62)}.land-memory-card strong{display:block;margin-bottom:.25rem}.land-memory-card p{margin:0;font-size:.88rem;line-height:1.55}
  ...
  function mount(win) {
      ...
      try {
        geo.getCurrentPosition = function (success, error, options) {
          ...
          nativeGetCurrentPosition((position) => {
            if (!gate.settle(token)) return;
            if (accuracyStatus(position) === 'insufficient') {
              if (typeof error === 'function') error({ code: 2, message: 'insufficient accuracy', accuracy: position.coords.accuracy });
              queueMicrotask(() => setStatus(`位置情報の精度が不足している（約${Math.round(position.coords.accuracy)}m）。安全な場所で再試行するか、模擬探索へ切り替えられる。`, 'warning'));
              return;
            }
    ...
    doc.addEventListener('click', (event) => {
      ...
      if (target) gate.invalidate();
      if (target && target.id === 'start-sim') setStatus('模擬探索へ切り替えた。遅れて届いたGPS結果は進行に使わない。', 'trace');
    }, true);
    ...
    if (collision && collisionDev && !doc.querySelector('#collision-live')) {
      ...
      live.addEventListener('click', () => {
        if (!win.navigator.geolocation || typeof win.navigator.geolocation.getCurrentPosition !== 'function') {
          setStatus('合流地点のGPS探索は使えない。DEV操作でも同じ発見transitionを確認できる。', 'warning');
          return;
        }
        win.navigator.geolocation.getCurrentPosition((position) => {
          ...
          if (result.status === 'discovered') {
            collisionDev.click();
            setStatus('二つの痕跡が交わる土地を発見した。GPSとDEVは同じ発見transitionを通った。', 'found');
            return;
          }
    ...
    function renderEnding() {
      ...
      surface.innerHTML = `
        ...
        <p class="ending-branch">${escapeHtml(completion.branchLabel)}: ${escapeHtml(completion.branchKey)}</p>
        <div class="land-memory-grid">${completion.memories.map((entry) => `<article class="land-memory-card" data-place="${escapeHtml(entry.id)}"><strong>${escapeHtml(entry.title)}</strong><p>${escapeHtml(entry.text)}</p></article>`).join('')}</div>
    ...
    if (resetButton) resetButton.addEventListener('click', () => {
      ...
      if (!confirmed) return;
      try { win.localStorage.removeItem(base.STORAGE_KEY); }
