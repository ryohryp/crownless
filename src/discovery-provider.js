(function (root, factory) {
  ...
  function normalizeGeographicFeature(feature) { const tags = tagsOf(feature); const types = []; const add = (type) => { if (!types.includes(type)) types.push(type); }; if (tags.natural === "water" || tags.waterway || tags.water || tags.landuse === "reservoir") add("water"); if (tags.bridge === "yes" || tags.bridge || tags.ford === "yes" || tags.highway === "ford") add("crossing"); if (tags.amenity === "place_of_worship" || tags.historic === "wayside_shrine" || tags.cemetery || tags.landuse === "cemetery") add("sacred"); if (tags.natural === "wood" || tags.landuse === "forest" || tags.leisure === "park") add("woods"); if (tags.railway === "station" || tags.public_transport === "station" || tags.highway === "traffic_signals" || tags.junction) add("road_hub"); if (tags.natural === "peak" || tags.natural === "ridge" || tags.natural === "hill" || tags.ele) add("height"); if (tags.natural === "coastline" || tags.place === "island") add("coast"); if (["city", "town", "village", "hamlet", "suburb", "neighbourhood", "quarter"].includes(tags.place)) add("settlement"); return { id: String((feature && feature.id) || "feature"), name: featureName(tags), types: FEATURE_ORDER.filter((type) => types.includes(type)) }; }
  function normalizeGeographicContext(features) { const seen = new Set(); const types = []; const namesByType = {}; (Array.isArray(features) ? features : []).forEach((feature) => { const item = normalizeGeographicFeature(feature); item.types.forEach((type) => { if (!seen.has(type)) { seen.add(type); types.push(type); } if (item.name && !namesByType[type]) namesByType[type] = item.name; }); }); return { types: FEATURE_ORDER.filter((type) => types.includes(type)), namesByType }; }
  function normalizeGeographicFeatures(features) { return normalizeGeographicContext(features).types; }
  const DISCOVERY_RULES = [
    { requires: ["water", "sacred"], signal: "水辺に、祈りの跡らしい石影が沈んでいる。", title: "沈んだ祠", risk: 3, palette: "water", kind: "event" }, { requires: ["water", "crossing"], signal: "渡り場の向こうで、烏が同じ場所を旋回している。", title: "血濡れの渡し場", risk: 3, palette: "water", kind: "encounter" }, { requires: ["woods", "sacred"], signal: "木立の奥から、集落のない方角で鐘が鳴った。", title: "苔むした聖域", risk: 2, palette: "woods", kind: "event" }, { requires: ["woods"], signal: "木々の向こうに細い煙が上がっている。", title: "森の野営地", risk: 2, palette: "woods", kind: "encounter" }, { requires: ["crossing"], signal: "古い道の狭まる場所に、人影が動かず立っている。", title: "見張られた辻", risk: 2, palette: "road", kind: "encounter" }, { requires: ["height"], signal: "高みの輪郭に、崩れた石組みが空を切っている。", title: "崩れた物見台", risk: 3, palette: "road", kind: "dungeon" }, { requires: ["coast"], signal: "潮の届かぬはずの場所に、濡れた木片が散っている。", title: "難破者の跡", risk: 2, palette: "water", kind: "event" }, { requires: ["road_hub", "settlement"], signal: "幾筋もの道が集まる先に、煤けた旗が見える。", title: "灰の街道宿", risk: 1, palette: "road", kind: "event" }, { requires: ["water"], signal: "水際に沿って、何か重いものを引きずった跡が続く。", title: "葦辺の巣穴", risk: 2, palette: "water", kind: "encounter" }, { requires: ["sacred"], signal: "風が止むたび、欠けた石から低い音が返ってくる。", title: "忘れられた石堂", risk: 2, palette: "road", kind: "event" }, { requires: ["road_hub"], signal: "道が交わる先だけ、土が黒く踏み固められている。", title: "黒土の辻", risk: 2, palette: "road", kind: "encounter" }, { requires: ["settlement"], signal: "人の気配はあるのに、煙の上がらない一角がある。", title: "閉ざされた路地", risk: 2, palette: "road", kind: "event" }
  ];
