...
class CrownlessRecipeExtension(Extension):
    ...
    def _run_once(self):
        ...
        try:
            ...
            base_path = self._resolve(repo_root, recipe["baseSnapshot"])
            editable_path = self._resolve(repo_root, recipe["editableSource"])
            ...
            # Exercise Krita's external-image layer path. Keep it hidden in the
            # editable source as provenance, so it cannot alter the runtime export.
